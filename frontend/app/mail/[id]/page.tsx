...
export default function MailDetailPage() {
  ...
  const emailId = typeof params.id === 'string' ? parseInt(params.id, 10) : null;
  ...
  const fetchEmail = async (pass: string) => {
      ...
      <div className="card">
        {isLoading && (
          <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--text-muted)' }}>
            Decrypting post-quantum encrypted message...
          </div>
        )}
        ...
        {emailDetail && (
          <div>
            <div className="card-title" style={{ alignItems: 'flex-start', flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <Lock size={20} color="#0284c7" />
                <span style={{ fontSize: '1.35rem' }}>{emailDetail.subject}</span>
              </div>
            ...
            >
              ...
              <Button
                variant="switch"
                onClick={() => fetchEmail(customPassphrase)}
